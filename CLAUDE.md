...
No summary stat cards above the table anymore (an earlier "Needs review" / "Last 30 days" tile strip was tried, then removed as noise the table itself already communicates).
...
### 9. Agent Summary: flat convince-to-approve bullets, animated "live" on open
...
On opening a customer's detail page, the summary **actually types out in real time**, bullet by bullet — the same live-streaming treatment as the AI-refine draft composer's "Polish" etc. buttons (`typeInto`), not a canned CSS trick. `renderAgentSummary()` renders each not-yet-typed bullet as an empty target (`[data-as-b="i"]`); `playAgentSummaryTyping()` + `typeIntoSelector()` stream plain text into it character-by-character, then swap in the final rich HTML (bold via `mdInline()`) once that bullet finishes. The box also carries the `.agent-summary.live` sweep/pulse glow (the same `.cmp-editor.generating` treatment, `aiSweep`/`aiPulse` keyframes) for the duration of the typing, mirroring the composer's "generating" state. State: `agentSummaryLive` (typing in progress) + `agentSummaryTyped` (bullet index → finished?) — the latter exists so an unrelated re-render mid-animation (e.g. clicking Pause) redraws already-typed bullets as-is instead of blanking them back to placeholders; `typeIntoSelector()` also re-queries its target by selector on every tick rather than caching the element, so it keeps typing into whatever the current DOM node is even across such a re-render.
