...
Each entry's label is derived from position, not a parallel array: first is always Trigger, last is always Recommendation, everything between is Key facts — and the label only prints once per slot (a second Key facts line doesn't repeat the caption). That's what lets the list flex between 3 and 4 lines without any per-scenario bookkeeping of which slot is which.
